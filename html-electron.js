...
exports = module.exports = HTML_Electron;
...
function HTML_Electron (runner) {
  ...
  const titles = runner.suite.suites
    ...
    .reduce((list, title, suiteIdx) => list
      .concat([title])
        .concat(runner.suite.suites[suiteIdx].suites
          .map(subSuite => '  ' + subSuite.title)
      ),
      []
    )
...
/**
 * Makes a URL, preserving querystring ("search") and ("hash") parameters.
 *
 * @param {string} s
...
function makeUrl (s) {
  ...
  return window.location.pathname 
      + (search ? search + '&' : '?') 
      + 'grep=' + encodeURIComponent(escapeRe(s))
